...
class ChadoGene(ChadoObject):
    ...
    def is_current_symbol(self, G1b_entry):
            log.info('Checking whether \'{}\' is the current symbol in Chado'.format(G1b_entry[FIELD_VALUE]))
            ...
            if symbol_name_from_results == G1b_entry[FIELD_VALUE]:
                log.info('\'{}\' matches the current symbol in Chado: \'{}\', FBgn: \'{}\'. Returning is_current = \'t\''.
                    format(G1b_entry[FIELD_VALUE], self.G1a_symbol_in_FB[FIELD_VALUE], self.gene.uniquename))
                return True
            else:
                log.info('\'{}\' does not match the current symbol in Chado: \'{}\', FBgn: \'{}\'. Returning is_current = \'f\''.
                    format(G1b_entry[FIELD_VALUE], self.G1a_symbol_in_FB[FIELD_VALUE], self.gene.uniquename))
                return False
